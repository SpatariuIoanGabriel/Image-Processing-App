...
bool Image::load(std::string imagePath)
{
	...
	if (inputLine.compare("P2") != 0) cerr << "Invalid image" << endl;

	// Second line is a comment and should be ignored
	getline(infile, inputLine);
	// Continue with a stringstream
	ss << infile.rdbuf();

	// Third line is the size of the image
	ss >> this->m_width >> this->m_height;
	cout << this->m_width << " columns and " << m_height << " rows" << endl;

	// Fourth line is the maximum pixel value
	int maxVal;
	ss >> maxVal;
